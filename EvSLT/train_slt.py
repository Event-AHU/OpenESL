...
# *transformers
from transformers import MBartForConditionalGeneration, MBartTokenizer,MBartConfig, MBart50TokenizerFast
from transformers.models.mbart.modeling_mbart import shift_tokens_right

# *user-defined
from models_sign import SignModel
...
def get_args_parser():
    parser = argparse.ArgumentParser('Event Sign Language Translation', add_help=False)
    ...
    parser.add_argument('--config', type=str, default='./configs/config_event_sign.yaml')
...
def main(args, config):
    ...
    if mixup_active:
        mixup_fn = Mixup(
            ...
            label_smoothing=0.2, num_classes=2506)
...
def train_one_epoch(args, model: torch.nn.Module, criterion: nn.CrossEntropyLoss,
    ...
    for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # src_input = dict_to_device(src_input, device)
        # tgt_input = dict_to_device(tgt_input, device)

        # from fvcore.nn import FlopCountAnalysis
        # flops = FlopCountAnalysis(model, (src_input, tgt_input))
...
def evaluate(args, dev_dataloader, model, model_without_ddp, tokenizer, criterion,  config, UNK_IDX, SPECIAL_SYMBOLS, PAD_IDX, device):
    ...
    with torch.no_grad():
        ...
        for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(dev_dataloader, 10, header)):
            ...
            output = model_without_ddp.generate(src_input, max_new_tokens=150, num_beams = 4, decoder_start_token_id=tokenizer.lang_code_to_id['zh_CN'])
    ...
    if utils.is_main_process() and utils.get_world_size() == 1 and args.eval:
        with open(args.output_dir+'/tmp_pres.txt','w') as f:
            for i in range(len(tgt_pres)):
                # f.write(tgt_pres[i]+'\n')
                f.write(tgt_pres[i].replace(' ','').replace('',' ').strip()+'\n')
        with open(args.output_dir+'/tmp_refs.txt','w') as f:
            for i in range(len(tgt_refs)):
                # f.write(tgt_refs[i]+'\n')
                f.write(tgt_refs[i].replace(' ','').replace('',' ').strip()+'\n')
        print('\n'+'*'*80)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('Event Sign Language Translation', parents=[get_args_parser()])
    ...
    if utils.is_main_process():
        wandb.init(project='Event-SLT',config=config)
